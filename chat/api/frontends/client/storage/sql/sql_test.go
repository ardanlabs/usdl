...
func TestContacts(t *testing.T) {
	...
	err = db.InsertMessage(user.ID, app.Message{
		Name:    "test_message_name",
		Content: []byte("test_message_content"),
	})
	assert.NoError(t, err)
...
func TestInsertMessage(t *testing.T) {
	...
	assert.Equal(t, []byte("test_message_content_2"), user.Messages[1].Content)
